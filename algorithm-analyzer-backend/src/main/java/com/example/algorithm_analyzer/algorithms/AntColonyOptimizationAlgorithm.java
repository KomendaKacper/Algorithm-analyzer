...
public class AntColonyOptimizationAlgorithm extends AbstractAlgorithm {
    ...
    private String selectNextElement(Problem problem, String current, List<String> candidates, Map<String, Double> pheromones, AcoParameters params) { Map<String, Double> probabilities = new HashMap<>(); double totalProbability = 0.0; for (String candidate : candidates) { double pheromoneLevel = pheromones.getOrDefault(problem.getPheromoneKey(current, candidate), 1.0); double heuristicValue = problem.getHeuristicValue(current, candidate); double probability = Math.pow(pheromoneLevel, params.alpha()) * Math.pow(heuristicValue, params.beta()); probabilities.put(candidate, probability); totalProbability += probability; } if (totalProbability == 0) return candidates.get(new Random().nextInt(candidates.size())); double randomValue = Math.random() * totalProbability; double cumulativeProbability = 0.0; for (Map.Entry<String, Double> entry : probabilities.entrySet()) { cumulativeProbability += entry.getValue(); if (randomValue <= cumulativeProbability) return entry.getKey(); } return candidates.get(candidates.size() - 1); }
    ...
    private record AcoParameters(int antCount, int iterations, double alpha, double beta, double evaporationRate, double pheromoneDeposit, double elitistWeight, boolean enableFullPheromoneTracking) { AcoParameters(Map<String, Object> params) { this(((Number) params.getOrDefault("antCount", 20)).intValue(), ((Number) params.getOrDefault("iterations", 1000)).intValue(), ((Number) params.getOrDefault("alpha", 0.7)).doubleValue(), ((Number) params.getOrDefault("beta", 0.7)).doubleValue(), ((Number) params.getOrDefault("evaporationRate", 0.2)).doubleValue(), ((Number) params.getOrDefault("pheromoneDeposit", 0.3)).doubleValue(), ((Number) params.getOrDefault("elitistWeight", 1.5)).doubleValue(), (Boolean) params.getOrDefault("enableFullPheromoneTracking", false)); } }
