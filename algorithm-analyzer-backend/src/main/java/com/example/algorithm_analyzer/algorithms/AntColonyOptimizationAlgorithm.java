...
public class AntColonyOptimizationAlgorithm extends AbstractAlgorithm {
    ...
    protected Map<String, String> getSpecificMetricLabels() {
        // Zostawiamy tylko te, które mają sens jako wykres pojedynczej wartości
        return Map.of(
                "pheromoneStats", "🐜 Statystyki Feromonów"
        );
    }
    ...
    protected ExecutionResult solve(Problem problem, Map<String, Object> algorithmParameters) {
        ...
        for (int iter = 0; iter < params.iterations(); iter++) {
            ...
            // --- NOWA LOGIKA: Dynamiczne budowanie mapy metryk ---
            ...
            // Dołącz pełną mapę feromonów tylko jeśli użytkownik tego zażądał
        ...
        return new ExecutionResult(finalSolution, bestFitnessGlobal, iterationResults, Map.of("pheromones", pheromones));
    }

    private IterationStats calculateIterationStats(List<AntSolution> solutions, boolean maximize) {
        if (solutions.isEmpty()) return new IterationStats(null, maximize ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY, Double.NaN, maximize ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY);
    ...
    private Map<String, Double> initializePheromones(Problem problem) {
        ...
        elements.forEach(from -> elements.forEach(to -> { if (!from.equals(to)) pheromones.put(problem.getPheromoneKey(from, to), 1.0); }));
        if (problem.getStartElement() == null) elements.forEach(element -> pheromones.put(problem.getPheromoneKey(null, element), 1.0));
        return pheromones;
    }
    ...
    private double calculateEdgeDiversity(List<List<String>> paths) {
        ...
        paths.forEach(path -> { for (int i = 0; i < path.size() - 1; i++) allEdges.add(path.get(i) + "->" + path.get(i + 1)); });
    ...
    private double calculateSetDiversity(List<List<String>> solutions) {
        ...
        double totalDistance = 0; int pairCount = 0;
        for (int i = 0; i < solutionSets.size(); i++) {
            for (int j = i + 1; j < solutionSets.size(); j++) {
                ...
                Set<String> intersection = new HashSet<>(set1); intersection.retainAll(set2);
                Set<String> union = new HashSet<>(set1); union.addAll(set2);
    ...
    private record AcoParameters(int antCount, int iterations, double alpha, double beta, double evaporationRate, double pheromoneDeposit, double elitistWeight, boolean enableFullPheromoneTracking) {
        AcoParameters(Map<String, Object> params) {
            this(
                    ...
                    (Boolean) params.getOrDefault("enableFullPheromoneTracking", false)
            );
        }
    }
